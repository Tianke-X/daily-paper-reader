...
tags: ["query:pwt"]
score: 8.0
evidence: 为长时序推理代理提供状态自适应记忆
tldr: 长程智能体推理中，历史交互信息分散且仅在特定时刻相关，现有方法截断或压缩历史但未考虑状态适配。本文提出状态自适应记忆SAM，将交互整合为紧凑线索并保留原始轨迹页面，通过专家指导和强化学习优化记忆模块。在BrowseComp等基准上，SAM显著优于多种基线，表明显式记忆建模为长程推理提供有效基础。
source: arxiv
...
motivation: 现有方法未建模对过去交互的访问应如何随智能体状态变化，导致长程推理中信息利用效率低。
method: SAM框架通过紧凑记忆线索和意图驱动回忆重建历史，并利用专家监督和强化学习优化记忆模块，无需重训练基座模型。
result: 在BrowseComp、WideSearch等任务上，SAM一致超越多种基线，且适配不同智能体骨干。
conclusion: 显式状态自适应记忆建模为长程智能体推理提供简单有效的基础。
---

## 摘要
长时代理推理要求大型语言模型在包含思考、工具调用、观察和部分结论的长时间交互历史中行动。挑战不仅在于这些历史会变长，还在于当前决策所需的信息可能分散在相隔较远的步骤中，并且只有在后续才变得相关。现有方法通过截断交互历史、将其压缩为更短的替代品或检索其选定部分以供重用，来解决这一困难，但它们并未明确建模对过去交互的访问应如何适应代理不断变化的状态。相反，我们将长时推理归结为状态自适应记忆的问题。为此，我们提出了状态自适应记忆（State-Adaptive Memory，SAM），这是一个独立的框架，它将持续交互整合为紧凑的记忆线索，同时保留原始轨迹页面以供意图驱动的回忆。这些线索不被视为历史的替代品；相反，它们作为轻量级句柄，使代理能够根据当前需求重建时间上遥远的信息，而无需重新训练底层骨干网络。我们进一步通过专家引导的监督学习和强化学习优化记忆模块，使其与轨迹级效用对齐。在BrowseComp、BrowseComp-ZH、WideSearch和HLE上，SAM在各种代理骨干网络上持续优于强基线。我们的结果表明，显式记忆建模为长时代理推理提供了一个简单而有效的基础。
