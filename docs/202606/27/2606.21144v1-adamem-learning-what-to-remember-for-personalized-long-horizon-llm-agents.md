...
tldr: "针对LLM智能体长期记忆膨胀导致个性化QA准确率下降的问题，提出AdaMem方法，通过学习角色依赖的记忆策略，利用每周QA反馈以轻量级自我反思调整记忆，在AdaMem-Bench基准上实现QA准确率提升9.0%且内存减少9%。"
source: arxiv
...
motivation: 现有LLM智能体长期记忆系统试图记住所有事实，导致无关信息积累，降低个性化QA准确率。
method: AdaMem维护结构化的角色特定记忆策略，通过每周QA反馈进行补丁式自我反思和失败回滚，学习每个用户应记住的内容。
result: "在两个提取模型和两种反馈模式下，AdaMem相比均匀记忆基线Mem0，QA准确率提升9.0%，内存体积缩小9%。"
conclusion: AdaMem通过选择性记忆有效缓解记忆膨胀，提升长期个性化交互的问答准确性。
---

## 摘要
大型语言模型智能体的长期记忆系统通常试图“记住一切”，统一提取记忆以保留尽可能多的事实。然而，在生产中，推理成本和有限的上下文预算使得这种方法难以维持：除了将原始对话整合到记忆中之外，智能体还必须施加“写入控制”，高效地只保留每个用户实际关心的信息。否则，长期个性化交互会遭受“记忆膨胀”，其中无关的琐事挤占有用信息，并逐渐侵蚀问答准确性。我们认为，值得记住的内容取决于角色，并提出AdaMem（自适应记忆），一种从反馈中学习为每个用户记住什么的方法。AdaMem维护一个结构化的、角色特定的记忆策略，并通过轻量级的、补丁式的自我反思步骤（带有失败回滚）从每周的问答反馈中优化该策略。为了研究这一设置，我们构建了AdaMem-Bench，一个模拟数周交互并逐周进行问答的基准测试。在两种提取模型和两种反馈模式下，与统一的Mem0基线相比，AdaMem将问答准确性提升了高达+9.0%，同时将记忆体积缩小了9%。
